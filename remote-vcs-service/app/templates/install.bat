...
if not defined DICODE_BASE_URL set "DICODE_BASE_URL=__BASE_URL__"
...
if not defined VERSION (
  powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $m=Invoke-RestMethod -Uri '!DICODE_BASE_URL!/dicode/pkg/latest.json' -TimeoutSec 30; $m.tag_name } catch { exit 1 }" > "!TMPDIR!\version.txt"
...
set "URL=!DICODE_BASE_URL!/dicode/pkg/!VERSION!/!ARCHIVE!"
...
if "!MODIFY_PATH!"=="1" (
  powershell -NoProfile -ExecutionPolicy Bypass -Command "$d='!DICODE_INSTALL_DIR!'; $p=[Environment]::GetEnvironmentVariable('Path','User'); $a=$p -split ';'; if ($d -notin $a) { [Environment]::SetEnvironmentVariable('Path',(($a + $d | Where-Object { $_ }) -join ';'),'User') }; [Environment]::SetEnvironmentVariable('DICODE_BASE_URL','!DICODE_BASE_URL!','User')"
)
